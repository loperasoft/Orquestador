 ...
 * @author c12976a
 */
public class TransformadorEntradaBusinessXPM implements TransformadorEntrada, Serializable {

	/** Serial UID */
	private static final long serialVersionUID = 2713044853821281621L;
	private static Fechas FECHAS = new Fechas();
	private static Logger logger = LogManager.getLogger(TransformadorEntradaBusinessXPM.class);
	private static TransformadorEntradaBusinessXPM solitario = new TransformadorEntradaBusinessXPM();
	...
	private TransformadorEntradaBusinessXPM() {
		super();
	}

	/**
	 * @return TransformadorEntradaBusinessXPM
	 */
	public static TransformadorEntradaBusinessXPM getTransformadorEntradaBusinessXPM(Map<String, String> salarios) {
	...
	public static Persona procesar(JsonObject jsonData) throws AdaptadorEntradaException {
		...
		try {
			...
			if (jsonData.has("inquiryFootprint")){
			...
			if (identification != null) {
				...
				if (identification.has("officialSourceValidationStatus")) {
				...
				if (identification.has("issuingCityCodeAsReported") && persona.getTipoId().equals(1)) {
					...
					if (identification.has("idStatus")) {
			...
			if (jsonData.has("bestAccounts")) {
				...
				accounts.forEach(jsonAccount -> {
					...
					if (account.has("businessLineCode") && account.has("accountType") &&
						...
						if (frequentBehaviour != null && frequentBehaviour.has("businessBureauEvent")) {
							String novedad = frequentBehaviour.get("businessBureauEvent").getAsString();
						...
						for (FrequentBehaviourDTO pojo : dates) {
							...
							if (dateCounter <= fechaCalculo) {
								...
								if (frequentBehaviour != null && frequentBehaviour.has("businessValueBalanceOverdue")) {
									String businessValueBalanceOverdue = frequentBehaviour.get("businessValueBalanceOverdue").getAsString();
									Double saldoMora = businessValueBalanceOverdue != null
											? Double.valueOf(businessValueBalanceOverdue) : null;
						...
						if (account.has("normalizedBusinessBehaviourVectorDA")) {

							String businessBehaviourVectorNormalized = account.get("normalizedBusinessBehaviourVectorDA")
									.getAsString();
						...
						if (account.has("businessDebtorNotFoundDate")) {
							String businessDebtorNotFoundDate = account.get("businessDebtorNotFoundDate").getAsString();

							if (!businessDebtorNotFoundDate.contains("-999999999")) {

								AdjetivoDTO adjetivoDTO = new AdjetivoDTO();
								adjetivoDTO.setFecha(new Date(Long.valueOf(businessDebtorNotFoundDate)));
						...
						if (account.has("businessAccountAtCollectionDate")) {
							String businessAccountAtCollectionDate = account.get("businessAccountAtCollectionDate").getAsString();

							if (!businessAccountAtCollectionDate.contains("-999999999")) {

								AdjetivoDTO adjetivoDTO = new AdjetivoDTO();
								adjetivoDTO.setFecha(new Date(Long.valueOf(businessAccountAtCollectionDate)));
						...
						if (account.has("businessLineDisconnectedDate")) {
							String businessLineDisconnectedDate = account.get("businessLineDisconnectedDate").getAsString();

							if (!businessLineDisconnectedDate.contains("-999999999")) {

								AdjetivoDTO adjetivoDTO = new AdjetivoDTO();
								adjetivoDTO.setFecha(new Date(Long.valueOf(businessLineDisconnectedDate)));
						...
						if (account.has("businessPrebusinessLegalCollectionDate")) {
							String businessPrebusinessLegalCollectionDate = account.get("businessPrebusinessLegalCollectionDate").getAsString();

							if (!businessPrebusinessLegalCollectionDate.contains("-999999999")) {

								AdjetivoDTO adjetivoDTO = new AdjetivoDTO();
								adjetivoDTO.setFecha(new Date(Long.valueOf(businessPrebusinessLegalCollectionDate)));
						...
						if (account.has("businessLegalCollectionDate")) {
							String businessLegalCollectionDate = account.get("businessLegalCollectionDate").getAsString();

							if (!businessLegalCollectionDate.contains("-999999999")) {

								AdjetivoDTO adjetivoDTO = new AdjetivoDTO();
								adjetivoDTO.setFecha(new Date(Long.valueOf(businessLegalCollectionDate)));
						 ...
						 */
						if (bs && br && bb && ba && bn && bt && bc && bf && bv && bp && bm){
